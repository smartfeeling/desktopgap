...
OAuthManager.prototype.api_call = function (func, params, needs_auth, format_json, is_post, callback) {
    console.log();
    var that = this;
    ...
    req.open(method, callUrl + '?' + params, true);
    req.setRequestHeader('X-Requested-With', null);
    req.setRequestHeader('Accept', null);
    req.onreadystatechange = function () {
        if (req.readyState == 4 || req.readyState == 'complete') {
            if (req.status == 200) {
                var data = format_json ? JSON.parse(req.responseText) : req.responseText;
                callback(data);
            }
            else
                that.options.onError(req);
        }
    };
    req.send(null);
};
